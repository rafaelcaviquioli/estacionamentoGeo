        ...
        <div class="panel panel-default">
            ...
            <div class="panel-body">
                <div class="row clearfix">
                    <div class="col-md-12 column">
                        <form id='filtro' class="form-horizontal" role="form" method="POST" action="?acao=AdmCategoria&search">
...
<div class="row clearfix">
    <div class="col-md-12 column">
        <table class="table table-hover">
            ...
            <?php if (count($this->request->query->get('categorias'))) { ?>
            <tbody>
                <?php
                foreach ($this->request->query->get('categorias') as $categoria) {
                    ?>
                    <tr>
                        ...
                        <td class='col-sm-3 col-xs-3'>
                            <div class="hidden-xs">
                                <a href="?acao=AdmCategoria&id=<?= $categoria->getId(); ?>" class="btn btn-success">Editar</a>
                                <button type="button" data-toggle="modal" data-target="#myModal<?= $categoria->getId(); ?>" class="btn btn-danger">Excluir</button>
                            </div>
                            <div class="visible-xs">
                                <a href="?acao=AdmCategoria&id=<?= $categoria->getId(); ?>" class="btn btn-success"><span class="glyphicon glyphicon-pencil"></span></a>
                                <button type="button" data-toggle="modal" data-target="#myModal<?= $categoria->getId(); ?>" class="btn btn-danger">
                                    <span class="glyphicon glyphicon-remove"></span></button>
                            </div>
                            <form name="delete" action="?acao=AdmCategoria&id=<?= $categoria->getId(); ?>" method="POST">
        ...
        <?php } ?>
        <a href="?acao=AdmCategoria" class="btn btn-success">
